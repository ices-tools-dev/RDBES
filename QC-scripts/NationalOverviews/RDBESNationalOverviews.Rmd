...
```{r CLoffweightSciweight, fig.dim= c(12, 8)}
CLweightS <- CL %>%
  group_by(CLspecCode) %>%
  summarise(CLoffWeight = sum(CLoffWeight),
            CLsciWeight = sum(CLsciWeight)) %>%
  gather(CLweightSource, CLweight, -c(CLspecCode))

ggplot(CLweightS, aes(CLspecCode, CLweight, fill = CLweightSource)) +
...
```{r CLoffweightSciweightSource, fig.dim= c(12, 8)}
CLweightSource <- CL %>%
  group_by(CLspecCode, CLdSouSciWeig) %>%
  summarise(CLsciWeight = sum(CLsciWeight)) 

ggplot(CLweightSource, aes(CLspecCode, CLsciWeight, fill = CLdSouSciWeig)) +
  geom_bar(position="dodge", stat="identity") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
...
CLWeightG <- CL %>%
  group_by(CLspecCode, CLarea) %>%
  summarise(CLoffWeight = sum(CLoffWeight))

myplots <- list()
for(i in unique(CLWeightG$CLspecCode)){
  myplots[[i]] <- 
    ggplot(subset(CLWeightG, CLspecCode == i), aes(CLarea, CLoffWeight))+
...
CLVel <- CL %>%
  group_by(CLarea, CLspecCode, CLvesLenCat) %>%
  summarise(CLoffWeight = sum(CLoffWeight))

myplotsVel <- list()
for(i in unique(CLVel$CLspecCode)){
  myplotsVel[[i]] <- 
    ggplot(subset(CLVel, CLspecCode == i), aes(CLarea, CLoffWeight, fill = CLvesLenCat)) +
  geom_bar(stat = "identity") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
}
```
...
```{r CLValS, fig.dim= c(12, 8)}

CLValS <- CL %>%
  group_by(CLarea, CLspecCode, CLdSouLanVal) %>%
  summarise(CLoffWeight = sum(CLoffWeight))

ggplot(subset(CLValS), aes(CLspecCode, CLoffWeight, fill = CLdSouLanVal)) +
  geom_bar(position="dodge", stat="identity") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
...
myplotsVal <- list()
for(i in unique(CL$CLspecCode)){
  myplotsVal[[i]] <- 
    ggplot(subset(CL, CLspecCode == i), aes(x= CLoffWeight, y=CLtotOffLanVal)) + geom_point()
}
```
...
CLLoc <- CL %>%
  group_by(CLarea, CLspecCode, CLloc) %>%
  summarise(CLoffWeight = sum(CLoffWeight))

myplotsLoc <- list()
for(i in unique(CLLoc$CLspecCode)){
  myplotsLoc[[i]] <- 
    ggplot(subset(CLLoc, CLspecCode == i), aes(CLarea, CLoffWeight, fill = CLloc)) +
    geom_bar(stat = "identity") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
}
```
...
CLMap <- CL %>%
  distinct(CLstatRect, CLspecCode) %>%
...
for(i in unique(CLMap$CLspecCode)){
  ...
  myplotsMap[[i]] <- 
    ...
    theme_bw() +
    geom_sf(data=world)+
    geom_point(data = subset(CLMap, CLspecCode == i), aes(lon, lat), size=2, colour="red") +
