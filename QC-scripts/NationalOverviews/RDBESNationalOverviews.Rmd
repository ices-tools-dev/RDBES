...
```{r CLCEcon}
# Just to test - REMOVE
CL$CLmonth <- sample(1:12, nrow(CL), replace = TRUE)
CE$CEMonth <- sample(1:12, nrow(CE), replace = TRUE)
...
CE <- mutate(CE, CEGearActivity = ifelse(CEGear %in% c("FIX", "FAR", "FPN", "FPO", "FWR", "FYK", "GEN", "GNF", "GN", "GNC", "GND", "GNS", "GTN", "GTR", "GES", "LH", "LHM", "LHP", "LL", "LLD", "LLF", "LLS", "LTL", "LVD", "LVS", "LX", "LN", "LNB", "LNP", "LNS"), "Passive", ifelse(CEGear %in% "NK", "NK", "Active")))
```
...
CLRectErr <- CLRect %>%
  ...
  rename(SubmittedRectangleCLtable = CLstatRect,
         SubmittedAreaCLtable = CLarea,
         ActualICESArea = ICESArea)
